        ...
        <div class="row g-4">
            <div class="col-md-6">
                <div class="profile-card">
                    ...
                    <div class="badges-earned mt-3">
                        <h5>Badges Earned</h5>
                        <div class="d-flex flex-wrap">
                            <div v-for="badge in badges" :key="badge.id" class="badge b1 me-2 mb-2">
                                <span>{{ badge.icon || '🏆' }}</span> {{ badge.name }}
                            </div>
                        </div>
                    </div>
                    <div class="regions-explored mt-3">
                        <h5>Regions Explored</h5>
                        <div class="d-flex flex-wrap">
                            <div v-for="region in regions" :key="region.id" class="badge b2 me-2 mb-2">
                                <span>{{ region.flag_emoji || '🌍' }}</span> {{ region.name }}
                            </div>
                        </div>
                    </div>
  ...
  methods: {
    ...
    async fetchRegions() {
      try {
        ...
        // Fetch user passport to get explored regions
        const passportRes = await fetch('/api/user/passport', {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        })
        const passport = await passportRes.json();
        const exploredKeys = passport.regions_explored ? Object.keys(passport.regions_explored) : [];
        // Only show regions that are explored
        this.regions = allRegions.filter(r => exploredKeys.includes(String(r.id)));
      } catch (error) {
        this.regions = []
      }
    }
